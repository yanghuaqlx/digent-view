...
一键导出整个知识库的链接结构，生成 DNA CSV 文件——你的数智体神经架构的一张定格快照。

**世界上没有两个一模一样的知识库。** 就像没有两个人脑拥有完全相同的连接组（connectome），每个 Obsidian 库的链接结构都是独一无二的。Digent DNA 文件记录了这种结构：每一个神经元、每一个空突触类、每一条突触链接，以及它们在三维空间中的精确位置。
...
| DNA-X / DNA-Y / DNA-Z | 节点在空间中的 3D 坐标（与 Noda 导出使用同一坐标系） |
| 源 | 突触链接的源节点（仅连线行有值） |
| 目标 | 突触链接的目标节点（仅连线行有值） |
